...
static const unsigned char KEYS_N[6][KEY_BYTESIZE] = {
	// Touhou 13.5
	{
...
Rsa::Rsa(FX::FXFile &file) : ifile(&file)
{
	this->init();
}
...
void Rsa::init()
{
	...
	RSA_N = mirvar(0);
	bytes_to_big(KEY_BYTESIZE, (const char *)KEYS_N[0], RSA_N);
}
...
bool Rsa::initRsaPublicKey(const unsigned char *crypted_sample)
{
	...
	for (auto &it : KEYS_N)
	{
		mirkill(RSA_N);
		RSA_N = mirvar(0);
		bytes_to_big(KEY_BYTESIZE, (const char *)it, RSA_N);
...
bool Rsa::read32(void *buffer, size_t size)
{
	...
	long ret = this->ifile->readBlock((char *)tmp, 64);
	if (ret != 64)
		return false;
